...
const comps = [
  ["openice", "Открытый лед", 180, 70, 20, 1, 230], ["yard", "Первые показательные", 450, 120, 40, 2, 560],
  ["school", "Школьная лига", 1000, 230, 80, 4, 1250], ["district", "Кубок района", 2200, 420, 150, 7, 2800],
  ["city", "Кубок города", 5200, 900, 330, 12, 6600], ["winter", "Зимнее первенство", 12000, 1800, 700, 20, 15000],
  ["region", "Региональное первенство", 26000, 3800, 1500, 34, 33000], ["federation", "Кубок федерации", 55000, 7600, 3200, 58, 70000],
  ["juniors", "Юниорский финал", 100000, 13500, 6500, 90, 128000], ["challenge", "Национальный челлендж", 180000, 23000, 12000, 130, 230000],
  ["nationals", "Чемпионат страны", 300000, 38000, 21000, 190, 385000], ["cupfinal", "Финал Кубка", 470000, 62000, 36000, 270, 600000],
  ["grandprix", "Финал Гран-при", 700000, 98000, 62000, 390, 900000], ["euro", "Континентальный старт", 950000, 145000, 98000, 560, 1220000],
...
const sponsors = [
  ["local", "Локальный спортмагазин", 2500, 0, 4, 0.01], ["rink", "Ледовый дворец", 15000, 40, 18, 0.02],
  ["media", "Медиа-партнер", 60000, 130, 60, 0.03], ["gear", "Бренд экипировки", 180000, 320, 150, 0.04],
  ["fashion", "Премиальный модный дом", 500000, 700, 420, 0.06]
...
function initial() {
  return {
    ...
    program: { spin: true }, sponsors: {}, prestige: 0, news: ["БЛИК: новый фигурист вышел на лед. Начинается путь."],
...
function sponsorBonus(s) { return sponsors.filter((x) => s.sponsors?.[x.id]).reduce((a, x) => ({ coinsSec: a.coinsSec + x.coinsSec, reward: a.reward + x.reward }), { coinsSec: 0, reward: 0 }); }

function derived(s) {
  const prestigeBoost = 1 + (s.prestige || 0) * 0.08;
  const sp = sponsorBonus(s);
  const d = { click: 2 * prestigeBoost, autoSkill: 0, fansClick: 1, autoFans: 0, score: programScore(s), reward: sp.reward, maxEnergy: 100, sponsorCoins: sp.coinsSec };
  upgrades.forEach((u) => {
...
function Ice({ s, d, next, float, train, reset }) {
  return <div className="space-y-4"><Card><div className="mb-3 flex items-start justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[0.24em] text-[#00FF00]">Текущий этап</p><h1 className="mt-1 text-2xl font-black text-white">{d.stage.name}</h1><p className="mt-1 text-xs leading-relaxed text-white/55">{d.stage.note}</p></div><button type="button" onClick={reset} className="flex h-10 w-10 shrink-0 items-center justify-center border border-[#00FF00]/35 bg-white/[0.03] text-[#00FF00]"><icons.reset size={17} /></button></div><div className="grid grid-cols-2 gap-2"><Pill icon={icons.star} label="Мастерство" value={fmt(s.skill)} /><Pill icon={icons.money} label="Бюджет" value={`${fmt(s.coins)} ₽`} /><Pill icon={icons.fans} label="Фанаты" value={fmt(s.fans)} /><Pill icon={icons.cup} label="Репутация" value={fmt(s.rep)} /></div></Card><Card><div className="mb-4 space-y-3"><Bar value={s.energy} max={d.maxEnergy} label={`Энергия: ${Math.floor(s.energy)} / ${d.maxEnergy}`} />{next ? <Bar value={s.skill - d.stage.min} max={next.min - d.stage.min} label={`До «${next.name}»: ${fmt(Math.max(0, next.min - s.skill))}`} /> : <Bar value={100} max={100} label="Карьерная вершина достигнута" />}</div><div className="relative flex min-h-[330px] items-center justify-center overflow-hidden border border-white/10 bg-black p-5"><div className="absolute inset-0 opacity-25"><div className="absolute left-1/2 top-1/2 h-[430px] w-[430px] -translate-x-1/2 -translate-y-1/2 rounded-full border border-[#00FF00]/40" /><div className="absolute left-1/2 top-1/2 h-[290px] w-[290px] -translate-x-1/2 -translate-y-1/2 rounded-full border border-white/25" /><div className="absolute left-0 top-1/2 h-px w-full bg-[#00FF00]/40" /><div className="absolute left-1/2 top-0 h-full w-px bg-[#00FF00]/30" /></div>{float.map((f) => <div key={f.id} className="pointer-events-none absolute z-20 animate-pulse border border-[#00FF00] bg-[#00FF00] px-4 py-2 text-sm font-black text-black shadow-xl">{f.text}</div>)}<button type="button" onClick={train} className="relative z-10 flex h-56 w-56 flex-col items-center justify-center rounded-full border-2 border-[#00FF00] bg-white text-black shadow-[0_0_70px_rgba(0,255,0,0.28)] active:scale-95"><icons.ice size={45} /><span className="mt-3 text-2xl font-black">Тренировка</span><span className="mt-2 text-[10px] font-black uppercase tracking-[0.18em] text-black/60">+{d.click.toFixed(1)} за тап</span></button></div></Card></div>;
}
...
function More({ s, d, prestige }) { return <div className="space-y-4"><Career s={s} d={d} /><Achievements s={s} /><Card><Title small="БЛИК" title="Новости" icon={icons.spark} /><div className="space-y-2">{(s.news || []).map((x, i) => <div key={`${x}-${i}`} className="border border-white/10 bg-white/[0.035] px-3.5 py-3 text-xs leading-relaxed text-white/65">{x}</div>)}</div></Card><Card><Title small="Финал" title="Престиж" icon={icons.crown} /><p className="mb-3 text-xs leading-relaxed text-white/60">После вершины льда можно начать карьеру заново с бонусом наследия. Сейчас престиж: {s.prestige || 0}.</p><button type="button" onClick={prestige} disabled={s.skill < 1300000} className={`w-full border px-4 py-3 text-xs font-black uppercase ${s.skill >= 1300000 ? "border-[#00FF00] bg-[#00FF00] text-black" : "border-white/10 bg-white/5 text-white/35"}`}>Начать новую легенду</button></Card></div>; }
...
export default function FigureSkatingClicker() {
  ...
  function train() { buzz("medium"); let msg = null; setS((old) => { const base = freshTasks(tick(old).s); const dd = derived(base); const crit = Math.random() < 0.08 + Math.min(base.rep / 5000, 0.08); const mod = base.energy >= 1 ? 1 : 0.25; const gain = dd.click * mod * (crit ? 3 : 1); const fanGain = dd.fansClick * mod * (crit ? 2 : 1); const stageIndex = Math.max(0, stages.findIndex((x) => x.name === dd.stage.name)); const coinGain = Math.max(1, Math.floor((stageIndex + 1) * mod)); let ns = { ...base, skill: base.skill + gain, fans: base.fans + fanGain, coins: base.coins + coinGain, energy: clamp(base.energy - 1, 0, dd.maxEnergy), updated: Date.now() }; ns = bumpTask(bumpTask(bumpTask(ns, "trains", 1), "fans", fanGain), "coins", coinGain); pop(crit ? `Идеальный прыжок +${Math.floor(gain)}` : `+${Math.floor(gain)}`); if (crit) ns = addLog(ns, "Чистый выезд после сложного прыжка. Зал ожил."); const r = applyAch(ns); if (r.got.length) msg = `Достижение: ${r.got[0]}`; return r.s; }); if (msg) setToast(msg); }
  ...
  function compete(c) { let msg = null; setS((old) => { const base = freshTasks(tick(old).s); const dd = derived(base); if (base.skill < c.req || base.energy < 30) { buzz(); return base; } buzz("heavy"); const ratio = (base.skill + base.rep * 80 + dd.score * 120 + Math.random() * c.diff * 0.45) / c.diff; const place = ratio >= 1.35 ? "золото" : ratio >= 1.08 ? "серебро" : ratio >= 0.88 ? "бронза" : "4 место"; const mult = ratio >= 1.35 ? 1.25 : ratio >= 1.08 ? 0.9 : ratio >= 0.88 ? 0.65 : 0.35; const first = base.done[c.id] ? 1 : 1.4; const m = mult * first * (1 + dd.reward); const coins = Math.floor(c.coins * m), fans = Math.floor(c.fans * m), rep = Math.floor(c.rep * mult * first); msg = `${c.title}: ${place}`; let ns = { ...base, coins: base.coins + coins, fans: base.fans + fans, rep: base.rep + rep, energy: clamp(base.energy - 30, 0, dd.maxEnergy), done: { ...base.done, [c.id]: true }, updated: Date.now() }; ns = bumpTask(bumpTask(bumpTask(ns, "starts", 1), "fans", fans), "coins", coins); ns = addNews(ns, `БЛИК: старт «${c.title}» завершен. Результат — ${place}.`); const r = applyAch(addLog(ns, [`${c.title}: ${place}. +${fmt(coins)} ₽, +${fmt(fans)} фанатов, +${rep} репутации.`, "Прокат завершен. Опыт и сезонный прогресс засчитаны."])); if (r.got.length) msg = `Достижение: ${r.got[0]}`; return r.s; }); if (msg) setToast(msg); }
  function reset() { buzz("heavy"); try { window.localStorage.removeItem(KEY); } catch {} setS(initial()); setTab("ice"); setFloat([]); setToast("Новая карьера началась"); }
  ...
  function doPrestige() { setS((old) => { if (old.skill < 1300000) return old; const next = initial(); return { ...next, prestige: (old.prestige || 0) + 1, news: [`БЛИК: новая легенда начинается с наследием ${(old.prestige || 0) + 1}.`] }; }); setTab("ice"); }

  return <div className="min-h-[100dvh] overflow-hidden bg-black text-white" style={{ "--tg-accent": BRAND.green, paddingTop: "calc(env(safe-area-inset-top) + 10px)" }}><div className="pointer-events-none fixed inset-0 opacity-70"><div className="absolute -left-24 top-[-120px] h-80 w-80 rounded-full bg-[#00FF00]/10 blur-3xl" /><div className="absolute right-[-160px] top-20 h-96 w-96 rounded-full bg-white/5 blur-3xl" /><div className="absolute bottom-[-180px] left-1/4 h-96 w-96 rounded-full bg-[#00FF00]/10 blur-3xl" /></div><main className="relative mx-auto max-w-md px-3 pb-[calc(env(safe-area-inset-bottom)+104px)]"><header className="mb-3 flex items-center justify-between gap-3 border border-[#00FF00]/25 bg-black px-4 py-3 shadow-2xl"><div><div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.22em] text-[#00FF00]"><icons.ice size={13} /> Mini App</div><h1 className="text-xl font-black text-white">Путь фигуриста</h1></div><BrandLogo className="h-10 w-auto max-w-[118px]" /></header>{tab === "ice" ? <Ice s={s} d={d} next={next} float={float} train={train} reset={reset} /> : null}{tab === "tasks" ? <Tasks s={s} claim={claim} /> : null}{tab === "events" ? <Events s={s} compete={compete} /> : null}{tab === "shop" ? <><Shop s={s} buy={buy} /><div className="mt-4"><Program s={s} toggle={toggleProgram} /></div><div className="mt-4"><Sponsors s={s} sign={signSponsor} /></div></> : null}{tab === "more" ? <More s={s} d={d} prestige={doPrestige} /> : null}</main><Nav tab={tab} setTab={setTab} />{toast ? <div className="fixed inset-x-3 bottom-[calc(env(safe-area-inset-bottom)+86px)] z-50 mx-auto max-w-sm border border-[#00FF00] bg-[#00FF00] p-4 text-center text-sm font-black text-black shadow-2xl">{toast}</div> : null}{finale ? <div className="fixed inset-x-3 top-[calc(env(safe-area-inset-top)+12px)] z-40 mx-auto max-w-sm border border-[#00FF00] bg-white px-4 py-3 text-black shadow-2xl"><div className="flex items-center gap-3"><icons.crown /><div><p className="text-[10px] font-black uppercase tracking-[0.2em] text-black/50">Финал</p><p className="text-sm font-black">Ты стал легендой льда</p></div></div></div> : null}</div>;
}
