...
const teamMembers = [
  ["headCoach", "Главный тренер", "Собирает систему подготовки и ускоряет рост мастерства.", 900, 1.72, 25, { autoSkill: 0.22, click: 0.18, formGain: 0.015 }],
  ["choreographer", "Хореограф", "Усиливает программу, артистизм и интерес аудитории.", 1400, 1.74, 25, { score: 0.65, fansClick: 0.16, reward: 0.004 }],
  ["psychologist", "Психолог", "Помогает держать форму и выдерживать давление стартов.", 2100, 1.76, 22, { maxEnergy: 1.8, formHold: 0.000045, formGain: 0.02 }],
  ["analyst", "Видеоаналитик", "Разбирает прокаты и добавляет точность в подготовку.", 3200, 1.78, 22, { score: 0.9, autoSkill: 0.08 }],
...
const teamCost = (m, lvl) => Math.floor(m.base * Math.pow(m.scale, lvl));
...
function initial() {
  return {
    ...
    program: { spin: true }, sponsors: {}, team: Object.fromEntries(teamMembers.map((m) => [m.id, 0])), prestige: 0, trainingMode: "jumps", playerName: "Фигурист БЛИК", form: 55,
...
function derived(s) {
  ...
  const d = { click: 2 * prestigeBoost * formBoost, autoSkill: 0, fansClick: 1, autoFans: 0, score: programScore(s), reward: sp.reward + Math.min(Math.sqrt(Math.max(0, s.fans)) / 4200, 0.28) + Math.min(Math.sqrt(Math.max(0, s.rep)) / 160, 0.22) + Math.max(0, (s.form || 0) - 50) / 600, maxEnergy: 100, sponsorCoins: sp.coinsSec, fanBoost: sp.fanBoost, repBoost: sp.repBoost, formBoost, formGain: 0, formHold: 0, sponsorMult: 1 };
  upgrades.forEach((u) => {
    const lvl = s.ups?.[u.id] || 0;
    Object.entries(u.effect).forEach(([k, v]) => { d[k] += v * lvl; });
  });
  teamMembers.forEach((m) => {
    const lvl = s.team?.[m.id] || 0;
    Object.entries(m.effect).forEach(([k, v]) => {
      if (k === "sponsorMult") d.sponsorMult += v * lvl;
      else d[k] = (d[k] || 0) + v * lvl;
    });
  });
  d.sponsorCoins *= d.sponsorMult;
...
function normalize(raw) {
  ...
  const s = {
    ...
    team: Object.fromEntries(teamMembers.map((m) => [m.id, clamp(Math.floor(n(raw.team?.[m.id])), 0, m.max)])),
...
function tick(s, now = Date.now()) {
  ...
  return applyAch({ ...x, skill: x.skill + d.autoSkill * sec, fans: x.fans + d.autoFans * sec, coins: x.coins + d.sponsorCoins * sec, energy: clamp(x.energy + 2.6 * sec, 0, d.maxEnergy), form: clamp((x.form || 55) - Math.max(0.0008, 0.0025 - d.formHold) * sec, 0, 100), updated: now });
}
...
const TESTS_OK = (() => {
  ...
  const teamTest = normalize({ team: { headCoach: 3 } }).team.headCoach === 3 && derived({ ...initial(), team: { ...initial().team, headCoach: 3 } }).autoSkill > derived(initial()).autoSkill;
  ...
  return paths && formTest && teamTest && nameTest && momentTest && dailyTest && s.skill === 500 && s.coins === 1000 && s.energy <= derived(s).maxEnergy && Math.abs(a.skill - 1.75) < 0.001 && b.s.coins === 310 && tabs.length === 5;
})();
...
export default function FigureSkatingClicker() {
  ...
  function train() { buzz("medium"); let msg = null; setS((old) => { const base = freshTasks(tick(old).s); const dd = derived(base); const mode = trainingModes.find((x) => x.id === base.trainingMode) || trainingModes[0]; const crit = Math.random() < 0.08 + Math.min(base.rep / 5000, 0.08); const mod = base.energy >= 1 ? 1 : 0.25; const gain = dd.click * mode.skillMod * mod * (crit ? 3 : 1); const fanGain = dd.fansClick * mode.fansMod * mod * (crit ? 2 : 1); const repGain = mode.repGain ? mode.repGain * mod : 0; const stageIndex = Math.max(0, stages.findIndex((x) => x.name === dd.stage.name)); const coinGain = Math.max(1, Math.floor((stageIndex + 1) * mod)); let ns = { ...base, skill: base.skill + gain, fans: base.fans + fanGain, rep: base.rep + repGain, coins: base.coins + coinGain, form: clamp((base.form || 0) + 0.08 + d.formGain + (mode.id === "stamina" ? 0.18 : 0) + (crit ? 0.04 : 0), 0, 100), energy: clamp(base.energy - mode.energyCost, 0, dd.maxEnergy), updated: Date.now() }; ns = bumpTask(bumpTask(bumpTask(ns, "trains", 1), "fans", fanGain), "coins", coinGain); pop(crit ? `Идеальный прыжок +${Math.floor(gain)}` : `+${Math.floor(gain)}`); if (crit) ns = addLog(ns, "Чистый выезд после сложного прыжка. Зал ожил."); const r = applyAch(maybeMedia(ns, 0.025)); if (r.got.length) msg = `Достижение: ${r.got[0]}`; return r.s; }); if (msg) setToast(msg); }
  ...
  function improveTeam(member) {
    setS((old) => {
      const base = freshTasks(tick(old).s);
      const lvl = base.team?.[member.id] || 0;
      const c = teamCost(member, lvl);
      if (lvl >= member.max || base.coins < c) { buzz(); return base; }
      buzz("medium");
      const next = addNews(addLog({ ...base, coins: base.coins - c, team: { ...base.team, [member.id]: lvl + 1 }, updated: Date.now() }, `Команда усилена: ${member.title}, уровень ${lvl + 1}.`), `БЛИК: штаб усиливает роль «${member.title}».`);
      return applyAch(next).s;
    });
  }
